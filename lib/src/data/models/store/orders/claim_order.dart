...
class ClaimOrder {
  ...
  ClaimFulfillmentStatus fulfillmentStatus = ClaimFulfillmentStatus.notFulfilled;
  ...
  ClaimOrder.fromJson(Map<String, dynamic> json) {
    ...
    if (ClaimType.values.any((e) => e.value == (json['type'] ?? ''))) {
      type = json['type'] != null ? ClaimType.values.firstWhere((e) => e.value == (json['type'] ?? '')) : null;
    }
    paymentStatus = ClaimPaymentStatus.values
        .firstWhere((e) => e.value == (json['payment_status'] ?? ''), orElse: () => ClaimPaymentStatus.na);
    fulfillmentStatus = ClaimFulfillmentStatus.values.firstWhere((e) => e.value == (json['fulfillment_status'] ?? ''),
        orElse: () => ClaimFulfillmentStatus.notFulfilled);
    if (json['claim_items'] != null) {
      claimItems = <ClaimItem>[];
      json['claim_items'].forEach((e) => claimItems!.add(ClaimItem.fromJson(e)));
    }
    if (json['additional_items'] != null) {
      additionalItems = <LineItem>[];
      json['additional_items'].forEach((e) => additionalItems!.add(LineItem.fromJson(e)));
    }
